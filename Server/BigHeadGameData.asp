...
If actionName = "save" Then
    ...
    saveSql = _
        ...
        "USING (SELECT CAST(? AS VARCHAR(200)) AS OID, CAST(? AS TINYINT) AS GameKey, " & _
        "CAST(? AS TINYINT) AS PlayerSlot, CAST(? AS NVARCHAR(MAX)) AS DataJson, " & _
        "CAST(? AS INT) AS Points, CAST(? AS INT) AS ShabbatPoints, CAST(? AS INT) AS HanukkaPoints, CAST(? AS INT) AS PurimPoints) AS Source " & _
