...
using OmniSARTechnologies.LiteFPSCounter;
    ...
    {
        public GalleryLevelView[] items = new GalleryLevelView[0];
        ...
        public virtual void ShowItem(int number, bool animate)
        {
            if (animate)
            {
                if (infiniteLoop || (number >= 0 && number < items.Length))
                {
                    if (animateScrollCoroutine != null)
                    {
                        isScrollAnimating = false;
        ...
        public void AutoGenerateItems()
        {
            ...
            GalleryLevelView[] temp = new GalleryLevelView[items.Length + autoGenerateItemsCount];
            ...
            for (int i = 0; i < autoGenerateItemsCount; i++)
            {
                ...
                levelViewItem.levelName = "item " + (items.Length + 1 + i);
                ...
                temp[items.Length + i] = levelViewItem;
            }
        ...
        {
            GalleryLevelView[] levels = items;
#if UNITY_EDITOR
            Undo.RecordObject(this, "delete");
#endif

            items = new GalleryLevelView[0];
        ...
        private void Initialize()
        {
            ...
            if (items.Length == 0) return;
            mActiveItems = Mathf.Clamp(activeItems, 4, items.Length);
            ...
            endOfItemsScrollPosition = (mItemsWidth + spaceBetweenItems) * (items.Length);

            start_level = Mathf.Clamp(start_level, 0, items.Length - 1);
        ...
        private void OnMouseButtonDown()
        {
            ...
            if (!infiniteLoop || items.Length < mActiveItems)
            {
                start = Mathf.Clamp(start, 0, items.Length - 1);
                end = Mathf.Clamp(end, 0, items.Length - 1);
            }
        ...
        {
            if (isScrollable && isTouchOnGallery && !isTouchedOnSlider)
            {
        ...
        private IEnumerator AnimateScroll(int index)
        {
            ...
            LiteFPSCounter fpsCounterVar = FindObjectOfType<LiteFPSCounter>(); // andy fix for diffrent framerates
            int frame = (int)(fpsCounterVar.frameRate)/2;// andy fix to make animation work for diffrent framrates
            float step = 0;
        ...
        private void OnGalleryPositionChange()
        {
            ...
            else if (infiniteLoop && end - start == items.Length)
            {
                start += 1;
            }
            ...
            int siblingIndex = items.Length - (end - start + 1);
            ...
            for (int i = start; i <= end; i++)
            {
                ...
                if (spaceBetweenItems < 0)
                {
                    if (i == centerIndex)
                    {
                        items[tempI].transform.SetSiblingIndex(items.Length - 1);
                        siblingIndex = items.Length - 2;
                    }
        ...
        private void UpdateTextIndicator(float scroll, int centerIndex)
        {
            if (scroll < 0 && (Mathf.Abs(scroll) - (centerIndex * scrollStep) >= ((centerIndex + 1) * scrollStep) - Mathf.Abs(scroll)))
            {
                if (levelTitleFormat == LevelTitleFormat.NumberAndCount)
                    txtLevelTitle.text = items.Length + "/" + items.Length;
            ...
            {
                if (levelTitleFormat == LevelTitleFormat.NumberAndCount)
                    txtLevelTitle.text = ((centerIndex % items.Length) + 1) + "/" + items.Length;
                if (levelTitleFormat == LevelTitleFormat.Number)
                    txtLevelTitle.text = ((centerIndex % items.Length) + 1) + "";
                if (levelTitleFormat == LevelTitleFormat.LevelName)
                    txtLevelTitle.text = items[(centerIndex % items.Length)].levelName;
            }
        }
        ...
        private void HideNotActiveItems(int start, int end)
        {
            foreach (int i in activeItemsArray)
            {
                ...
                if (notInActives && i < items.Length)
                    items[i].gameObject.SetActive(false);
            }
        }
        ...
        {
            Vector2 pos1;
            if (items.Length <= 1) pos1 = Vector2.zero;
            else
            {
                centerIndex = (int)(scroll / scrollStep) + 1;
                if (!infiniteLoop && centerIndex >= items.Length) centerIndex = items.Length - 1;
                float previousPos = itemsMappedPostion[centerIndex - 1];

                if (!infiniteLoop || (centerIndex >= 0 && centerIndex < items.Length - 1))
                {
        ...
        {
            itemsMappedPostion = new float[items.Length];
        ...
        private float horizontalNormalizedPosition
        {
            get
            {
                ...
                {
                    int order = reverseOrder ? -1 : 1;
                    float value = ((scrollAxis == ScrollAxis.Horizontal) ? galleryChild.transform.localPosition.x : galleryChild.transform.localPosition.y) / -((mItemsWidth + spaceBetweenItems) * (items.Length - 1));
                    return value * order;
                }
                else
                    return 0;
