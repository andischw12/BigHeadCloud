...
public class TKCharacterController3D : MonoBehaviour

{
                ...
                if (awake)
                {
                    ...
                    {   //no grounded
                        ...
                        if (!flying && !climbing)
                        {
                            ...
                            rigid.AddTorque(Vector3.up * (Input.GetAxisRaw("Horizontal") * 0.0125f), ForceMode.VelocityChange);
                        }
                    }
                }
                else
    ...
    void Checkinput()
    {
        ...
        {
            if (myspeed > (runspeed-0.25f)) tospeed = sprintspeed;
        }
        ...
        //SITDOWN      
        Debug.DrawRay(trans.position + new Vector3(0.0f, 0.15f, 0f) - trans.forward * 0.05f, -trans.forward* 0.1f);
        Debug.DrawRay(trans.position + new Vector3(0.0f, 0.18f, 0f) - trans.forward * 0.05f, -trans.forward * 0.15f);

        if (Input.GetKeyDown(KeyCode.C) && forward == 0 )
        {
            if (Physics.Raycast(trans.position + new Vector3(0.0f, 0.15f, 0f) - trans.forward * 0.05f, -trans.forward, 0.1f)
                && !Physics.Raycast(trans.position + new Vector3(0.0f, 0.18f, 0f) - trans.forward * 0.05f, -trans.forward, 0.15f))
            {
                StartCoroutine("Waitfor", 1f);
                sitdown = true;
                anim.SetBool("sitdown", true);
                anim.CrossFade("sitdown",0.025f);
            }
            else 
            {
                anim.CrossFade("lookback",0.025f);
                StartCoroutine("Waitfor", 0.8f);
            }
        }
    ...
    void Checkground()
    {
        ...
        Debug.DrawRay(trans.position + new Vector3(0f, 0.05f, 0f) + dirforw * 0.1f, Vector3.down * 0.25f, Color.red);
        Debug.DrawRay(trans.position + new Vector3(0f, 0.05f, 0f) + dirforw * -0.07f, Vector3.down * 0.25f, Color.red);
        Debug.DrawRay(trans.position + new Vector3(0f, 0.05f, 0f) + dirside * 0.06f, Vector3.down * 0.25f, Color.red);
        Debug.DrawRay(trans.position + new Vector3(0f, 0.05f, 0f) + dirside * -0.06f, Vector3.down * 0.25f, Color.red);

        if (!jumping)
        {
            float dist;
            if (grounded) dist = 0.2f; else dist = 0.1f;
            if (Physics.Raycast(trans.position + new Vector3(0f, 0.05f, 0f), Vector3.down, dist) ||
              Physics.Raycast(trans.position + new Vector3(0f, 0.05f, 0f) + dirforw * 0.1f, Vector3.down, dist) ||
              Physics.Raycast(trans.position + new Vector3(0f, 0.05f, 0f) + dirforw * -0.07f, Vector3.down, dist) ||
              Physics.Raycast(trans.position + new Vector3(0f, 0.05f, 0f) + dirside * 0.06f, Vector3.down, dist) ||
              Physics.Raycast(trans.position + new Vector3(0f, 0.05f, 0f) + dirside * -0.06f, Vector3.down, dist))
                grounded = true;
            else grounded = false;
        }
    ...
    {
        active = false;
        yield return new WaitForSecondsRealtime(itime);
        active = true;        
    }
    IEnumerator Jump()
    {
