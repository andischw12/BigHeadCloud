...
public class MaleKid_ClothesAndSkeenMaker : Avater_ClothesAndSkeenMaker
{
    ...
    [SerializeField] public int chestMat;
    [SerializeField] public int legsMat;
    [SerializeField] public int feetMat;
    GameObject GOhead;
    GameObject[] GOfeet;
    GameObject[] GOhair;
    GameObject[] GOchest;
    ...
    public override void SetSpecificChest(int gm, int mat)
    {
        ...
        //set GM
        while (!isReady) { }
        do Nextchest(); while (gm != chest);
           
        //set mat
        do Nextchestcolor(0); while (chestMat != mat);
    ...
    {//
     ...
     //set GM
        while (!isReady) { }
        do Nextlegs(); while (gm != legs);
           
        //set mat
        do Nextlegscolor(0); while (legsMat != mat);
    ...
    public override void SetSpecificFeet(int gm, int mat)
    {
        ...
        // Getready();
        //set GM
        do Nextfeet(); while (gm != feet);
            
        //set mat
        do Nextfeetcolor(0); while (feetMat != mat);
    ...
    {
        foreach (GameObject gm in GOchest)
            gm.SetActive(false);
    ...
    {
        foreach (GameObject gm in GOlegs)
            gm.SetActive(false);
    ...
    {
        foreach (GameObject gm in GOfeet)
            gm.SetActive(false);
    ...
    {
        if (chest < 2)  chestMat= Setmaterials(GOchest, MATshirt, 1, todo);   
        if (chest == 2) chestMat = SetGOmaterials(GOchest[2], MATsweater, 1, todo);
        if (chest > 2) chestMat = Setmaterials(GOchest, MATtshirt, 1, todo);
    }
    public override void Nextlegscolor(int todo)
    {        
        if (legs < 3)  legsMat = Setmaterial(GOlegs, MATlegs, todo);
        if (legs > 2) legsMat = Setmaterials(GOlegs, MATlegs, 1, todo);
    }
    public override void Nextfeetcolor(int todo)
    {
        if (feet < 2) feetMat = Setmaterial (GOfeet, MATfeetA,  todo);
        if (feet == 2) feetMat = Setmaterial(GOfeet, MATfeetB,  todo);
    }
    ...
    public int Setmaterial(GameObject[] GO, Object[] MAT, int todo)
    {
        int GOindex = 0;
        int MATindex = 0;
    ...
    public int SetGOmaterial(GameObject GO, Object[] MAT, int todo)
    {
        int MATindex = 0;
        Material AUXmaterial;
    ...
    public int SetGOmaterials(GameObject GO, Object[] MAT, int matchannel, int todo)
    {
        int MATindex = 0;
        Material[] AUXmaterials;
