...
public class GirlTKPrefabMaker : Avater_ClothesAndSkeenMaker
{
    ...
    {
        if (chest < 2)  chestMat = Setmaterials(GOchest, MATdress, 1, todo);
        if (chest == 2 || chest == 3) chestMat= Setmaterials(GOchest, MATshirt, 1, todo);
        if (chest == 4 ) chestMat= Setmaterials(GOchest, MATsweater, 1, todo);
        if (chest > 4) chestMat= Setmaterials(GOchest, MATtshirt, 1, todo);
    }
    public override void Nextlegscolor(int todo)
    {
        if (legs == 0) legsMat = SetGOmaterials(GOlegs[0], MATdress, 1,todo);

        if (legs > 0 && legs < 4) legsMat = Setmaterial(GOlegs, MATlegs, todo);
        if (legs > 3) legsMat = Setmaterials(GOlegs, MATlegs, 1, todo);
    }
    public override void Nextfeetcolor(int todo)
    {
        ...
        if (feet == 3) feetMat = Setmaterial(GOfeet, MATfeetC, todo);
    }
