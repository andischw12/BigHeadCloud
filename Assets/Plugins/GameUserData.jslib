mergeInto(LibraryManager.library, {
    $BigHeadStorage: {
        ...
        readEnvelope: function (player) {
            ...
            try {
                ...
                // Old versions stored the Unity JSON directly, without an envelope.
                var migrated = {
                    data: raw,
                    points: 0,
                    shabbatPoints: 0,
                ...
                console.warn("BigHead: invalid local data for player " + player, error);
                return null;
            }
        },
        ...
        writeEnvelope: function (player, envelope) {
            try {
                ...
                console.warn("BigHead: local save failed", error);
                return false;
            }
        },
        ...
        requestSync: function (action, query) {
            try {
                ...
                console.warn("BigHead: server load unavailable", error);
                return null;
            }
        },
        ...
        enqueueSave: function (player, envelope) {
            ...
            var pump = function () {
                ...
                self.postForm(body).then(function (response) {
                    ...
                    console.warn("BigHead: server save deferred; local copy is safe", error);
                    self.queues[queueKey + ":running"] = false;
                });
            };
            pump();
    ...
    loadDataJS: function (player) {
        ...
        // An unsynchronised local mutation is newer by definition. Return and retry it.
    ...
    saveDataJS: function (data, player, points, shabbatPoints, hanukkaPoints, purimPoints) {
        ...
        // Browser storage is the first durable write; the server copy follows.
    ...
    saveWinnersJS: function (winner, loser) {
        ...
        BigHeadStorage.postForm(body).catch(function (error) {
            console.warn("BigHead: result log was not saved", error);
        });
    },
